...
export default function GroupsScreen() {
  ...
  const handleCreateGroup = async () => {
    if (!groupName.trim()) {
      Alert.alert('Erreur', 'Indique un nom de groupe');
      return;
    }
    ...
    if (group) {
      Alert.alert(
        'Groupe créé',
        `Code d’invitation à partager : ${group.invite_code}`
      );
  ...
  const handleJoinGroup = async () => {
    if (!inviteCode.trim()) {
      Alert.alert('Erreur', 'Saisis un code d’invitation');
      return;
    }
              ...
              <Card>
                <View style={styles.emptyState}>
                  ...
                  <Text style={styles.emptyText}>
                    Aucun groupe pour l’instant. Crée-en un ou entre un code.
                  </Text>
                </View>
              </Card>
            ) : (
            ...
            <Card style={styles.infoCard}>
              <View style={styles.infoRow}>
                ...
                <Text style={styles.infoText}>
                  Classement basé sur la constance. Enchaîne les séances pour
                  grimper.
                </Text>
              </View>
            </Card>
            ...
            {leaderboard.length === 0 ? (
              <Card>
                <View style={styles.emptyState}>
                  ...
                  <Text style={styles.emptyText}>
                    Pas encore de classement. Termine des séances pour apparaître ici.
                  </Text>
                </View>
              </Card>
            ) : (
...
const styles = StyleSheet.create({
  ...
  groupName: { color: '#F8FAFC', fontSize: 16, fontWeight: '600' },
  groupDescription: { color: '#64748B', fontSize: 13, marginTop: 2 },
  groupFooter: { marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: '#334155' },
  inviteCodeContainer: { flexDirection: 'row', alignItems: 'center' },
  inviteCodeLabel: { color: '#64748B', fontSize: 12 },
  inviteCode: { color: '#10B981', fontSize: 14, fontWeight: '600', marginLeft: 8, letterSpacing: 1 },
  infoCard: { marginBottom: 16, backgroundColor: 'rgba(59, 130, 246, 0.1)', borderColor: '#3B82F6' },
  infoRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  infoText: { flex: 1, color: '#94A3B8', fontSize: 14 },
